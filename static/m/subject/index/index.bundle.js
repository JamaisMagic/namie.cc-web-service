...
var mAxiosInstance = axios.create({
    // baseURL: '/',
    timeout: 20000
});
...
mAxiosInstance.interceptors.request.use(function (config) {
    ...
    config.headers['Authorization'] = (0, _blueimpMd2.default)(JSON.stringify(config.data), mAuthKey);
    return config;
});

function getAxiosInstance(authKey) {
    if (authKey) {
        mAuthKey = authKey;
    }
...
var mAxios = (0, _request.getAxiosInstance)();
...
new _vue2.default({
    ...
    methods: {
        ...
        shorten: function shorten(url) {
            var self = this;
            mAxios.post(_request.URL_PATH.SHORTEN, { url: url }).then(function (res) {
                return res.data || {};
            }).then(function (data) {
                if (data.code === 0) {
